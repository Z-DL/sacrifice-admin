          ...
          <el-row :gutter="20">
            <el-col :xs="24" :sm="24" :md="10">
              <el-form :inline="true" :model="formInline" class="demo-form-inline">
                ...
                <el-button type="success" size="small" @click="onSubmit('down')">上架</el-button>
              </el-form>
            </el-col>
...
<script>
  import {getGoodType, fetchStoreList, deleteGoods, upGoods,uploadImg, editGood} from '@/api/goods'
...
//        this.$refs.upload.submit();
      ...
      fetchData1(id, name) {
        fetchStoreList(id, name, this.currentPage, this.$store.getters.token).then(res => {
          if (res.code == 200) {
            this.tableData = []
      ...
      onSubmit(state) {
        ...
        if (state === 'down') {
          this.$confirm('此操作将上架您选中的商品, 是否继续?', '提示', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning'
          }).then(() => {
            let commodity_id = this.multipleSelection.map(v => v.commodity_id)
            upGoods(commodity_id, this.$store.getters.token).then(res => {
              console.log(res)
              if (res.code == 200) {
                this.fetchData1(this.formInline.category_id, this.formInline.commodity_name)
                this.$message({
